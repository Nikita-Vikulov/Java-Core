    ...
    private void readMessages() throws IOException {
        while (true) {
            ...
            switch (tokens[0]) {
                ...
                case "/r": {// /r <oldNick> <newNick>
                    if (tokens.length < 3) {
                        Message msg = new Message();
                        msg.setMessage("Не хватает параметров, необходимо отправить команду следующего вида: /r <старый_ник> <новый_ник> ");
                        this.sendMessage(msg);
                    }
                    String oldNick = tokens[1];
                    String newNick = tokens[2];
                    BaseAuthService.renameUserNick(oldNick, newNick);
                    break;
                }
            }
        }
